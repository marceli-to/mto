...
import { PhFolder, PhUsers, PhReceipt, PhCurrencyCircleDollar, PhSignOut, PhGear, PhQuestion, PhChartBar } from '@phosphor-icons/vue'
...
<template>
  <aside class="fixed left-0 top-0 min-h-screen w-64 bg-gray-100 flex flex-col">
    ...
    <div class="py-4 px-6">
      <router-link to="/">
        <svg xmlns="http://www.w3.org/2000/svg" width="433.855" height="49.858" viewBox="0 0 433.855 49.858" class="w-full max-w-[145px]">
    ...
    <!-- Navigation -->
    <nav class="flex-1 py-4 px-6 space-y-1">
      <router-link
        v-for="item in navigation"
        :key="item.route"
        :to="{ name: item.route }"
        class="flex items-center gap-3 py-4 text-sm text-gray-500 font-regular hover:text-black transition-all duration-200"
