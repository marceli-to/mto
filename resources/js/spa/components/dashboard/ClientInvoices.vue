...
const clientAcronym = computed(() => route.params.client)
...
const filteredInvoices = computed(() => {
  return invoices.value.filter(inv =>
    inv.client?.acronym === clientAcronym.value &&
    inv.state?.description !== 'cancelled'
  )
})
...
const clientName = computed(() => {
  const invoice = filteredInvoices.value[0]
  return invoice?.client?.name || clientAcronym.value
})
...
<template>
  <div>
    ...
    <div v-else-if="filteredInvoices.length === 0" class="text-center py-16">
      <div class="text-gray-400 mb-2">No invoices found for {{ clientAcronym }}</div>
    </div>
